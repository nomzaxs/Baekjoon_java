...
class Pokemon {
    ...
    Pokemon(int index, String name) {
        this.index = index;
        this.name = name;
    }
}

public class PN_1620 {
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < N; i++) {
            pokemons[i] = new Pokemon(i, br.readLine());
        }

        Pokemon[] sortPokemons = pokemons.clone();

        Arrays.sort(sortPokemons, new Comparator<Pokemon>() {
            @Override
        ...
        for (int m = 0; m < M; m++) {
            ...
            if (str.charAt(0) > '9') {
                ...
                sb.append(search(str, sortPokemons) + "\n");
            } else {
                sb.append(pokemons[Integer.parseInt(str) - 1].name + "\n");
            }
        }
    ...
    private static int search(String str, Pokemon[] sortPokemons) {
        int start = 0;
        int end = sortPokemons.length - 1;

        while (start <= end) {
            int mid = (end + start) / 2;

            if (str.equals(sortPokemons[mid].name)) {
                return sortPokemons[mid].index + 1;
            } else {
                if (str.compareTo(sortPokemons[mid].name) < 0) {
                    end = mid - 1;
                } else {
                    start = mid + 1;
                }
