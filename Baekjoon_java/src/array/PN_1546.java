...
public class PN_1546 {
    ...
    private static void fixScore(int[] scoreArray) {
        int maxScore = 0;       // 제일 큰 점수
        double tScore = 0;      // 전체 점수
        double[] dArray = new double[scoreArray.length];

        for(int i = 0; i < scoreArray.length; i++){
            dArray[i] = (double)scoreArray[i];
        ...
        for(int i = 0; i < scoreArray.length; i++){
            dArray[i] = ((dArray[i] / maxScore) * 100);
            tScore += dArray[i];
        }

        System.out.println((double)tScore/scoreArray.length);
    }
